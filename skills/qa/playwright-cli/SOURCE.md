...
- Grant: this skill's frontmatter sets `allowed-tools: Bash(playwright-cli:*) Bash(npx:*) Bash(npm:*)`. `npx <package>` fetches and executes arbitrary registry code, and the skill body itself calls `npm install -g @playwright/cli@latest`, so the grant is broad and is recorded here deliberately rather than absorbed.
