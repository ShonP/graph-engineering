...
Written for **ruff 0.16.8** (PyPI JSON, published 2026-09-16 — the latest release, and the
version `forge-libs` pins with `==`). Docs fetched 2026-09-23, all HTTP 200:

- https://docs.astral.sh/ruff/configuration/ - "Configuring Ruff"
- https://docs.astral.sh/ruff/settings/ - "Settings"
- https://docs.astral.sh/ruff/rules/banned-api/ - "banned-api (TID251)"
- https://docs.astral.sh/ruff/formatter/ - "The Ruff Formatter"

In-repo measurement: `docs/adr/0021-temporal-converter-and-test-env.md`, "The lint: which
`@dataclass` spellings ruff alone misses" — a thirteen-row matrix run against two real
configurations. Every rule below that says "measured" points there, and it is rung 1
evidence: code that was run, not a docs claim.
...
- One `[tool.ruff]` table in the project's `pyproject.toml`, not a separate `ruff.toml` and
  ...
  so two files is a silent override, not a merge. https://docs.astral.sh/ruff/configuration/
...
- **Ban the symbol.** Banning the `dataclasses` module also flags
  ...
  `# noqa`'d on day one.
- Measured coverage, from the thirteen-row matrix: the symbol ban catches **eight**
  spellings — `from`-import, module attribute, aliased symbol, aliased module, the
  `pydantic.dataclasses` pair, and the functional `make_dataclass` API. **Four escape both
  the symbol ban and the module ban**, and really are dataclasses at runtime:
  ...
  with no decorator in the file at all. A star import escapes the symbol ban only.
...
- Reach past ruff for the two spellings worth catching statically — a star import and a
  same-file inherited dataclass are both a short `ast` walk — and leave the genuinely
  dynamic ones to review.
...
- **`force-exclude` defaults to `false`, and that default is load-bearing.** Verbatim from
  the settings page: "Typically, Ruff will lint any paths passed in directly, even if they
  would typically be excluded." So a file on `extend-exclude` is skipped by the bare
  `ruff check` sweep and **still checked when named on the command line**.
...
- **A project that declares no `lint` entry point gets no lint from the hook, silently.**
  ...
  walks past a scriptless project file for this reason.
...
- **Banning a module when you meant a symbol.** It flags the legitimate sibling API
  (`dataclasses.replace`) and buys nothing the symbol ban does not already catch — measured,
  ADR 0021: the two configurations catch the same eight spellings.
...
- **A bare `# noqa`.** It disables every current and future rule on that line. Use
  `# noqa: <CODE>` with a comment naming the reason.
...
# 6. No bare `# noqa` anywhere: every suppression names its rule.
grep -rn '# noqa\([^:]\|$\)' src/ tools/ || echo "no bare noqa"
