...
more than one method block is refused (Bruno sends the last one), and so is a
path with `.` or `..` segments. Runs on Python 3.9+ (the macOS default).
"""
...
METHOD_BLOCK = re.compile(
    r"^(get|head|options|post|put|patch|delete|graphql)\s*\{(.*?)^\}", re.S | re.M
)
