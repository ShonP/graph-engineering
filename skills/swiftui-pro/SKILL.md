...
> **House-rule note:** This skill states general SwiftUI practice. Your project's deployment target, i18n convention and design tokens come from the repo's own rule packs, named in `.claude/graph-profile.yaml` under `rules` - read those before applying anything here. Gate any API newer than the project's deployment target behind `#available`, and verify the availability claim against Apple's official documentation rather than a third-party mirror. Every example string, color, spacing, radius and font size in this skill and its references is illustrative only: real code resolves strings through the project's i18n layer and visual values through its design tokens, never as literals.
...
- Read the deployment target from the project's rule packs; never assume the newest OS is the baseline. Newer APIs are fair game in review suggestions, but every one must be gated with `#available(...)` and carry a working fallback at the project's actual target. Verify any availability claim against Apple's official documentation before citing it.
- Target modern Swift concurrency (async/await, actors); do not require a specific Swift language-mode baseline beyond what the project already declares.
