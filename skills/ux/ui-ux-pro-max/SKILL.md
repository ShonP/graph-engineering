...
Third-party skill (vendored community pack). Where it conflicts with the consuming repo's own rules or design system, the house rules win. Concretely:

- **If the repo has a design system**, never use the visual-design domains (`color`, `typography`, `google-fonts`, `style`) to pick values - that output is reference-only; all colors/spacing/radii/fonts come from the house tokens. Do NOT introduce palettes, font pairings, or CSS variables from this skill into such a repo.
- **Never run `--design-system --persist` in a repo that already has a design system.**
- **What this skill is always for**: the UX-judgment domains - `ux` (placement, navigation, forms, feedback, touch), `product` (which patterns fit the product type), `landing`, `chart`, and `references/quick-reference.md` + `references/pro-rules.md`. Use them to decide WHERE a control goes, WHAT flow a task follows, and WHICH pattern fits - then implement with house tokens/components.
- Motion presets are reference-only where the repo names its own motion library. Respect reduced-motion always.
- A UX recommendation must land on every platform the profile names (parity rule).
